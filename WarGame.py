...
class Captain(Private):  # captain class, privatenin sub classı
    ...
    def move(self, togo, captain):
        trench_list[togo].append(captain)  # captani siper sözlüğünde yeni bir yere ekliyor
        if len(self.trench) != 0:  # ve önceki yerden siliyor
            trench_list[self.trench[-1]].remove(captain)
        if togo not in self.trench:
            self.trench.append(togo)  # eğer yeni bir siper ise kendi siper listesine ekleniyor
...
class Colonel(Private):  # colonel classı, private sub class
    ...
    def move(self, togo):  # colonel siper değiştir method
        if self.trench[-1] == togo:
            print(f'You are alreday in trench {togo}.')
        else:
            self.trench.append(togo)
            print(f'You are in trench {togo} now.')
